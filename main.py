...
def connect_to_db():
    try:
        conn = mysql.connector.connect(host="localhost",user="root",password="your_password",database="expense_tracker")
...
def create_tables():
    try:
        ...
        if not cursor.fetchone():
            cursor.execute("""CREATE TABLE expenses (id INT AUTO_INCREMENT PRIMARY KEY,amount DECIMAL(10, 2),category VARCHAR(50),date DATE,description TEXT);""")
        cursor.execute("SHOW TABLES LIKE 'recurring_expenses';")
        if not cursor.fetchone():
            cursor.execute("""CREATE TABLE recurring_expenses (id INT AUTO_INCREMENT PRIMARY KEY,amount DECIMAL(10, 2),category VARCHAR(50),frequency VARCHAR(20),next_due_date DATE,description TEXT);""")
        conn.commit()
        cursor.close()
        conn.close()
    except mysql.connector.Error as err:
